...
export interface PetriDishProps {
  ...
  onToggleSimulation?: () => void;  // Callback to toggle simulation
...
export interface GraphNode {
  ...
  vx?: number;
  vy?: number;
  fx?: number;  // Fixed x position (undefined means not fixed)
  fy?: number;  // Fixed y position (undefined means not fixed)
  color: string;
  size: number;
  isResistant: boolean;
  fitness: number;
