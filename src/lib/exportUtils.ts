...
export class SimulationExporter {
  ...
  /**
   * Export comparison report for multiple simulations
   */
  static async exportComparisonReport(
    simulations: Simulation[],
    format: ExportFormat = "json",
    onProgress?: ExportProgressCallback
  ): Promise<void> {
    try {
      onProgress?.(10, "Analyzing simulation differences...");
      ...
      const comparisonData: any = {
        metadata: {
          exportDate: new Date().toISOString(),
          version: "1.0",
          type: "simulation_comparison_report",
      ...
      onProgress?.(40, "Computing parameter differences...");
      ...
      if (simulations.length >= 2) {
        ...
        simulations.slice(1).forEach((sim, index) => {
          const simId = sim.id;
          differences[simId] = {};

          // Compare parameters
          if (baseSimulation.parameters && sim.parameters) {
            Object.keys(baseSimulation.parameters).forEach(key => {
              const baseValue = (baseSimulation.parameters as any)[key];
              const compareValue = (sim.parameters as any)[key];
              
              if (baseValue !== compareValue) {
                differences[simId][key] = {
                  base: baseValue,
                  current: compareValue,
                  difference: typeof baseValue === 'number' && typeof compareValue === 'number' 
        ...
        comparisonData.differences = differences;
      }

      onProgress?.(70, "Generating comparison report...");
      ...
      if (format === "json") {
        ...
        onProgress?.(100, "Comparison report ready!");
        this.downloadFile(jsonContent, filename, "application/json");
      } else {
        ...
        // Add header row
        csvData.push({
          type: "comparison_header",
          total_simulations: simulations.length,
          export_date: new Date().toISOString()
        });

        // Add simulation summary rows
        simulations.forEach((sim, index) => {
          csvData.push({
            ...
            initial_population: sim.parameters?.initialPopulation || 0,
            growth_rate: sim.parameters?.growthRate || 0,
            antibiotic_concentration: sim.parameters?.antibioticConcentration || 0,
            mutation_rate: sim.parameters?.mutationRate || 0
          });
        });

        onProgress?.(90, "Generating CSV comparison report...");
        
        const csvContent = this.convertToCSV(csvData);
        const timestamp = new Date().toISOString().slice(0, 19).replace(/[:-]/g, "");
        const filename = `comparison_report_${timestamp}.csv`;

        onProgress?.(100, "CSV comparison report ready!");
        this.downloadFile(csvContent, filename, "text/csv");
      }
    } catch (error) {
      console.error("Comparison export failed:", error);
      throw new Error(`Failed to export comparison report: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}
...
export const exportComparisonReport = SimulationExporter.exportComparisonReport.bind(SimulationExporter);
